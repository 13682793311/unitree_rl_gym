...
# 用来在环境中生成柏林噪声，并且部署到环境中
class TerrainPerlin:
    def __init__(self, cfg, num_envs):
        self.cfg = cfg
        self.env_length = cfg.terrain_length # 地形长度
        self.env_width = cfg.terrain_width # 地形宽度
        self.xSize = cfg.terrain_length * cfg.num_rows # 整个地形在x和y方向上的总尺寸
        self.ySize = cfg.terrain_width * cfg.num_cols # 
        self.tot_rows = int(self.xSize / cfg.horizontal_scale) # 离散采样时的行数和列数
        ...
        self.heightsamples_float = self.generate_fractal_noise_2d(self.xSize, self.ySize, self.tot_rows, self.tot_cols, **cfg.TerrainPerlin_kwargs) # 生成一个浮点类型的高度样本数组
        ...
        self.heightsamples = (self.heightsamples_float * (1 / cfg.vertical_scale)).astype(np.int16) # 将浮点高度值按照配置的垂直缩放因子进行缩放取整生成最终的高度图
        self.heightfield_raw_pyt = torch.tensor(self.heightsamples, device= "cpu") # 将高度图转换为一个cpu上的pytorch tensor
        ...
        self.vertices, self.triangles = terrain_utils.convert_heightfield_to_trimesh(   self.heightsamples,
                                                                                        ...
                                                                                        cfg.vertical_scale,
                                                                                        cfg.slope_treshold) # 调用isaacgym里的方法，将二维网格转化为三角网格数据
    # 在给定的二维网络中生成标准的perlin噪声
    @staticmethod
